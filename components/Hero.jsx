...
const Hero = () => {
    ...
    const currency = process.env.NEXT_PUBLIC_CURRENCY_SYMBOL || 'VNĐ'
            ...
            <div className='flex max-xl:flex-col gap-8 max-w-7xl mx-auto my-10'>
                <div className='relative flex-1 flex flex-col bg-green-200 rounded-3xl xl:min-h-100 group'>
                    <div className='p-5 sm:p-16'>
                        <div className='inline-flex items-center gap-3 bg-green-300 text-green-600 pr-4 p-1 rounded-full text-xs sm:text-sm'>
                            <span className='bg-green-600 px-3 py-1 max-sm:ml-1 rounded-full text-white text-xs'>TIN TỨC</span> Miễn phí vận chuyển cho đơn hàng trên VNĐ50! <ChevronRightIcon className='group-hover:ml-2 transition-all' size={16} />
                        </div>
                        <h1 className='text-3xl sm:text-5xl leading-[1.2] my-3 font-medium bg-gradient-to-r from-slate-600 to-[#A0FF74] bg-clip-text text-transparent max-w-xs  sm:max-w-md'>
                            Tiện ích bạn sẽ yêu. Giá cả bạn sẽ tin.
                        </h1>
                    ...
                    <Image className='sm:absolute bottom-0 right-0 md:right-10 w-full sm:max-w-sm' src={assets.hero_model_img} alt="Người mẫu đang trưng bày các tiện ích" />
                </div>
                <div className='flex flex-col md:flex-row xl:flex-col gap-5 w-full xl:max-w-sm text-sm text-slate-600'>
                    <div className='flex-1 flex items-center justify-between w-full bg-orange-200 rounded-3xl p-6 px-8 group'>
                        ...
                        <Image className='w-35' src={assets.hero_product_img1} alt="Hình ảnh của một sản phẩm nổi bật" />
                    </div>
                    <div className='flex-1 flex items-center justify-between w-full bg-blue-200 rounded-3xl p-6 px-8 group'>
                        ...
                        <Image className='w-35' src={assets.hero_product_img2} alt="Hình ảnh của một sản phẩm nổi bật được giảm giá" />
                    </div>
                </div>
            </div>
            <CategoriesMarquee />
